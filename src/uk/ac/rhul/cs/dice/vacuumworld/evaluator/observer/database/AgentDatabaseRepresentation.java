 ...
 * {@link VacuumWorldCleaningAgent} data with {@link VWMongoBridge}. </br>
...
public class AgentDatabaseRepresentation implements RefinedPerception {
	 ...
	 * @param cycleList
	 *            a of {@link CycleDataRepresentation CycleDataRepresentations}
	 *            used as a record of what the agent has done during its
	 *            lifetime. Note that this list should usually be empty when
	 *            using this class for 'to JSON' conversion unless the
	 *            {@link VacuumWorldCleaningAgent} has previously existed e.g.
	 *            it is being loaded into another session.
	 */
	public AgentDatabaseRepresentation(String id, VacuumWorldAgentType type, int sensors, int actuators, int totalCleans, int successfulCleans, CycleDataRepresentation[] cycleList) {
		this.id = id;
		this.type = type;
