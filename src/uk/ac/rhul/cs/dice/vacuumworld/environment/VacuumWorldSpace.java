...
public class VacuumWorldSpace extends EnvironmentalSpace {
	...
	getAllLocations().stream().filter(VacuumWorldLocation::isAnAgentPresent).forEach(location -> this.agents.put(location.getAgent().getId(), location.getAgent()));
    }

    @SuppressWarnings("unchecked")
    private static Map<LocationKey, Location> cast(Map<? extends LocationKey, ? extends Location> grid) {
	Set<Entry<LocationKey, Location>> locations = grid.entrySet().stream().map(entry -> (Entry<LocationKey, Location>) entry).collect(Collectors.toSet());
	Map<LocationKey, Location> toReturn = new HashMap<>();
	locations.forEach(location -> toReturn.put(location.getKey(), location.getValue()));
	...
	getGrid().entrySet().stream().filter(entry -> entry.getKey() instanceof VacuumWorldCoordinates && entry.getValue() instanceof VacuumWorldLocation).forEach(entry -> toReturn.put((VacuumWorldCoordinates) entry.getKey(), (VacuumWorldLocation) entry.getValue()));
	...
	getObservers().forEach(recipient -> notifyAgentOrUserSensors(recipient, sensorsToNotifyIds, result));
    }
    
    private void notifyAgentOrUserSensors(CustomObserver recipient, List<String> sensorsToNotifyIds, VacuumWorldActionResult result) {
