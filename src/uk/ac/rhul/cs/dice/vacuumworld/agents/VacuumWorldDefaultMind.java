...
public class VacuumWorldDefaultMind extends CyclingAutonomousAgentMind implements VacuumWorldMindInterface {
	...
	public void start(int perceptionRange, boolean canSeeBehind, Set<Action> availableActions) {
	...
	private Action decideFromPerception(VacuumWorldActionResult previousActionResult) {
	...
	private void removeCleanIfNecessary() {
		...
		for(Action a : this.availableActions) {
			if(a instanceof CleanAction || a.getClass().isAssignableFrom(CleanAction.class)) {
	...
	private void addMoveIfNecessary() {
		for(Action a : this.availableActions) {
			if(a instanceof MoveAction || a.getClass().isAssignableFrom(MoveAction.class)) {
				return;
			}
		}
	...
	public int hashCode() {
		...
		result = prime * result + ((availableActions == null) ? 0 : availableActions.hashCode());
		...
		result = prime * result + ((previousActionResult == null) ? 0 : previousActionResult.hashCode());
		return result;
	}
