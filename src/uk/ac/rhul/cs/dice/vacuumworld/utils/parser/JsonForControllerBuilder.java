...
/**
 * This class creates a ModelUpdate for the View (to be transmitted through the
 * Controller). Only the relevant locations are included and, for each location,
 * only the relevant information about agent/user/dirt are included. A location
 * is relevant if it contains an agent/dirt/user (in other words, if it is not
 * empty). For a VacuumWorldLocation, the relevant information are the x and y
 * coordinates, the agent (if present), the user (if present) and the dirt (if
 * present). For a VacuumWorldCleaningAgent, the relevant information are its
 * color and its facing direction. For a User, only the facing direction is
 * relevant. For a Dirt, only the color is relevant.
 * 
